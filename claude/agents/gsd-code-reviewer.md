...
You orchestrate reviewers and synthesize results. You do NOT perform deep code review yourself — you spawn sub-agents who do the actual review work and write findings directly to `docs/review/layers/`.
</role>
...
UI Component → API Client call → Route → Controller → Service/Repository → SP/Query → Tables
...
Run build verification (MANDATORY — do this yourself, not a sub-agent):
...
4. **docs/review/PRIORITIZED-TASKS.md**
   - Flat ordered list: Blocker → High → Medium → Low
