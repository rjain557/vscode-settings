...
- docs/spec/openapi.yaml — REQUIRED for API and client enhancement
- docs/spec/apitospmap.csv — REQUIRED for SP-Only verification
...
<critical_rules>
1. **DETECT FIRST** — Always run component detection before spawning agents
2. **SKIP UNDETECTED** — Only spawn agents where detected == true
3. **RESPECT DEPENDENCIES** — Database -> API -> MCP -> Presentation -> Testing
4. **PARALLEL WHERE SAFE** — Wave 4 components can run in parallel
5. **SP-ONLY ENFORCED** — All database access via stored procedures + Dapper
6. **CONTRACT REQUIRED** — Enhancement agents need frozen contracts for validation
</critical_rules>
