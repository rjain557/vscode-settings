...
**G1 — Database:** All SPs in API-SP Map exist, signatures match, idempotent scripts
**G2 — API:** All OpenAPI endpoints implemented, routes map to SPs, auth present
**G3 — UI/MCP:** All routes exist, typed clients generated, five states, no direct DB
**G4 — SpecSync:** OpenAPI <-> Controllers, API-SP Map <-> Implementation, tests pass
**G5 — Runtime:** Application builds, config valid, health endpoints respond
</step>
...
```markdown
# SDLC Phase {letter} — {phase_name} — {Entrance|Exit} Gate Validation

**Phase:** {letter} — {phase_name}
