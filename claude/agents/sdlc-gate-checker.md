...
1. Read the phase reference document from `~/.claude/get-shit-done/references/sdlc/phases/phase-{letter}.md`
...
- `.planning/sdlc/phase-{X}/` — phase-specific artifacts
- `/docs/spec/` — specifications (OpenAPI, apitospmap.csv, ui-contract)
...
Special validations:
- "SCG1 signed" → Check APPROVALS.md for SCG1 entries with TArch + Product + DoSE
- "CI passes" → Check for recent green build evidence
- "All tests pass" → Check for test result files
- "No P0/P1 issues" → Check issue logs for unresolved critical issues
</step>

<step name="generate_report">
Create GATE-CHECK.md using the template from `~/.claude/get-shit-done/templates/sdlc/gate-check.md`
...
**"Phase X complete"** → Check SDLC-STATE.md for phase X status = "complete"
**"Artifact approved"** → Check APPROVALS.md for matching approval record
**"Artifact exists"** → Glob for the file, Read to verify non-empty
**"CI passes/green"** → Check for build artifacts or CI result files
**"No P0/P1 issues"** → Grep issue logs for unresolved critical items
**"Environment ready"** → Check for environment config files or health check evidence
**"Sign-off obtained"** → Check APPROVALS.md for required role sign-offs
**"100% coverage"** → Parse apitospmap.csv and compare against OpenAPI endpoints
**"Tests pass"** → Check for test result files with no failures
</phase_criteria_patterns>
