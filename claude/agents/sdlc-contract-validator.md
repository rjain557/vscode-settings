...
1. **OpenAPI Specification** (docs/spec/openapi.yaml) — API surface definition
2. **API-SP Map** (docs/spec/apitospmap.csv) — Endpoint to SP mapping (100% coverage required)
3. **UI Contract** (docs/spec/ui-contract.csv) — Screen definitions
4. **DB Plan** (docs/spec/db-plan.md) — Database schema plan

These must be perfectly consistent with implementation:
5. **Controllers** (src/Server/*/Controllers/) — Must match OpenAPI paths/methods
6. **DTOs** (src/Server/*/Models/) — Must match OpenAPI schemas AND SP result shapes
7. **Services** (src/Server/*/Services/) — Must call correct SPs per API-SP Map
8. **Stored Procedures** (db/sql/procedures/) — Must match API-SP Map names/params
9. **Typed Clients** (src/Client/*/api/) — Must match OpenAPI (no drift)
</contract_chain>
