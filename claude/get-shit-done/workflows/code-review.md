...
```bash
INIT=$(node C:/Users/rjain/.claude/get-shit-done/bin/gsd-tools.js init code-review 2>/dev/null || echo '{}')
```
...
For EACH feature/endpoint, trace:
  UI Component → API Client method → HTTP Route → Controller action → Service/Repository method → SP/Query → Database tables
...
Run build verification (MANDATORY — do this in the orchestrator, not a sub-agent):
...
- 70-79: C (Fair — needs attention before production)
- 50-69: D (Poor — significant work needed)
- 0-49: F (Critical — major rework required)
...
1. {risk — severity, component, description}
2. ...
...
All findings organized by severity (Blocker → High → Medium → Low).
...
```markdown
# Developer Handoff — Code Review Findings
...
<developer_regression_cases>

## Developer Regression Cases (2026-02-25)

Known runtime issues discovered during developer testing. Future reviews MUST check for these patterns.

### Case 1: API Response Shape Mismatch
- **Symptom**: `threads.find is not a function` crash at runtime
- **Root Cause**: `Chat.tsx:62` casts API response with `as ChatThread[]` but API returns wrapped object `{ data: [...] }`, not a raw array
- **Files**: `src/Client/technijian-spa/src/components/screens/chat/Chat.tsx`, API hooks returning paginated responses
- **Checklist Items**: CHECKLIST_FRONTEND > API Client Alignment > runtime shape validation, no unsafe `as Type[]` casts
- **Verification**: Search for `as Type[]` on any API response; must use Array.isArray guard or unwrap pagination wrapper

### Case 2: Claim Extraction Fallback Without Logging
- **Symptom**: User resolution failures -- controllers silently fall through claim chain
- **Root Cause**: `TcaiControllerBase.cs:14-24` tries `sub` -> `oid` -> `NameIdentifier` but logs nothing when primary claim missing. `AuthController.cs:622` has different fallback order.
- **Files**: `TcaiControllerBase.cs`, `AuthController.cs`, `TenantValidationMiddleware.cs`
- **Checklist Items**: CHECKLIST_DOTNET_BACKEND > Controller Layer > claim extraction WARN logging, centralized utility
- **Verification**: Grep for `FindFirst` or `FindFirstValue` in controllers; all must route through shared utility with WARN logging

### Case 3: X-Tenant-ID Header Extraction Inconsistency
- **Symptom**: Assistants endpoint errors -- empty string instead of null for missing header
- **Root Cause**: 8 controllers use `Request.Headers["X-Tenant-ID"].ToString()` (returns empty string), 13 use `.FirstOrDefault()` (returns null)
- **Files**: All 21 controllers in `src/Server/Technijian.Api/Controllers/`
- **Checklist Items**: CHECKLIST_DOTNET_BACKEND > Controller Layer > single utility method, no ToString() on StringValues
- **Verification**: `grep 'Request.Headers\["X-Tenant-ID"\]' --include="*.cs" -r src/Server/` must return zero matches in controllers

### Case 4: TenantValidationMiddleware False 403
- **Symptom**: Users with zero tenant assignments get 403 (Forbidden) instead of 401 (Unauthorized)
- **Root Cause**: `TenantValidationMiddleware.cs:68` checks `!allowedTenantIds.Contains(requestedTenantId)` which is true when list is empty, returning 403 instead of 401
- **Files**: `src/Server/Technijian.Api/Middleware/TenantValidationMiddleware.cs:62-78`
- **Checklist Items**: CHECKLIST_AUTH > Auth Status Code Semantics > 401 for zero assignments, 403 for wrong tenant
- **Verification**: Test user with zero assignments gets 401; user requesting wrong tenant gets 403

### Case 5: SQL SET Options Missing
- **Symptom**: SP failures when interacting with indexed views or computed columns
- **Root Cause**: All SPs have `SET NOCOUNT ON` but none have `SET QUOTED_IDENTIFIER ON` or `SET ANSI_NULLS ON`
- **Files**: All 5 procedure files in `db/sql/procedures/`
- **Checklist Items**: CHECKLIST_SQL_DATABASE > Stored Procedures > three-line SET preamble required
- **Verification**: Every SP must have SET NOCOUNT ON + SET QUOTED_IDENTIFIER ON + SET ANSI_NULLS ON

### Case 6: Admin Sidebar Role Visibility
- **Symptom**: Admin sidebar not visible for `technijian_employee` users
- **Root Cause**: `AppShell.tsx:105` defaults `currentTenantRole = 'user'`, making `canAccessAdmin` false for `technijian_employee` since it requires `currentTenantRole === 'admin'`
- **Files**: `src/Client/technijian-spa/src/components/layout/AppShell.tsx:98-163`, `TCAIApp.tsx`
- **Checklist Items**: CHECKLIST_FRONTEND > Routing & Navigation > role x tenant-role test matrix, default prop testing
- **Verification**: Test all 8 combinations (4 roles x 2 tenant roles); technijian roles must always see admin sidebar
...
- [ ] All API response mappers have runtime shape validation (Zod, io-ts, or manual guards); type assertions with `as` forbidden on API responses
- [ ] No unsafe `as Type[]` casts on API hook return values -- use array guards (Array.isArray) before .find/.map/.filter
...
- [ ] Role-based UI visibility has test matrix covering all role x tenant-role combinations (technijian_admin, technijian_employee, client_admin, client_user x admin, user)
- [ ] Optional props with role-based defaults are tested explicitly (default values must not hide functionality)
...
- [ ] Claim extraction (UserId, TenantId, Email) must log at WARN when primary claim is missing and fallback is used
- [ ] All claim extraction must use centralized utility (TcaiControllerBase properties or shared helper); no inline User.FindFirst chains in individual controllers
- [ ] All X-Tenant-ID extraction must use a single utility method (TcaiControllerBase.GetHeaderTenantId or centralized helper); no direct Request.Headers["X-Tenant-ID"] access
- [ ] No ToString() on StringValues for header extraction (returns empty string instead of null); always use FirstOrDefault() + null check
...
- [ ] Every SP must begin with SET NOCOUNT ON; SET QUOTED_IDENTIFIER ON; SET ANSI_NULLS ON (all three required)
- [ ] SP preamble validated across all procedure files (procedures.sql, council_api_procedures.sql, phase_79_parity_aliases.sql, phase_e_stored_procedures.sql)
...
- [ ] No check-then-insert patterns (SELECT then INSERT) that fail under concurrent load
- [ ] User/table creation uses MERGE statement for atomic upsert
- [ ] SPs are idempotent (safe to call multiple times with same result)
- [ ] Error 2627 (unique constraint) is handled gracefully where race conditions possible
- [ ] Authentication/user lookup SPs use HOLDLOCK or SERIALIZABLE where needed
...
- [ ] SSO callback is idempotent (handles duplicate callbacks without 500 errors)
- [ ] SSO user creation uses atomic MERGE or handles race conditions
- [ ] SSO callback stored procedures don't have check-then-insert race conditions

**Auth Status Code Semantics:**
- [ ] Return 401 for zero tenant assignments (user exists but no access); return 403 only when user has assignments but not for requested tenant
- [ ] Claim extraction across controllers must use identical fallback method (no mixed sub/oid/NameIdentifier chains)
- [ ] X-Tenant-ID header extraction across all controllers must use identical method (no mix of FirstOrDefault() and ToString())
- [ ] Middleware HTTP status semantics: 401 = identity problem or no assignments; 403 = wrong resource

**Database Stored Procedures:**
- [ ] No check-then-insert patterns that cause race conditions under concurrent load
- [ ] User/table creation uses MERGE statement or proper serialization
- [ ] Concurrent request handling is idempotent (same result regardless of retry count)
