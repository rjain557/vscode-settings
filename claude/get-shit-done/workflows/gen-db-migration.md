...
- ALTER: dbo.Conversations -- modify column Title nvarchar(200) → nvarchar(500)
...
db/migrations/
  config/
    ...
    type-mappings.json                 # SQL Server ↔ PostgreSQL type mappings
...
- Map types: `UNIQUEIDENTIFIER` → `UUID`, `NVARCHAR` → `VARCHAR`, `DATETIME2` → `TIMESTAMPTZ`
- Map syntax: `IF NOT EXISTS` → `CREATE TABLE IF NOT EXISTS`, `CREATE OR ALTER` → `CREATE OR REPLACE`
