...
src/Server/{ProjectName}/
  Services/Webhooks/
    Outbound/
      ...
      WebhookService.cs                 # Event → webhook dispatch
...
**Delivery pipeline:**
1. Event occurs → create WebhookDelivery record
2. Background worker picks up pending deliveries
3. POST payload to subscriber URL with signature header
4. On success (2xx) → mark delivered
5. On failure → retry with exponential backoff (1min, 5min, 30min, 2hr, 12hr)
