...
```
GET    /api/users          → getUsers(filters?: UserFilters): Promise<UserResponseDto[]>
GET    /api/users/{id}     → getUser(id: string): Promise<UserResponseDto>
POST   /api/users          → createUser(dto: CreateUserDto): Promise<UserResponseDto>
PUT    /api/users/{id}     → updateUser(id: string, dto: UpdateUserDto): Promise<UserResponseDto>
DELETE /api/users/{id}     → deleteUser(id: string): Promise<void>
```
</step>
