<purpose>
Generate integration tests for your API endpoints using xUnit and WebApplicationFactory. Creates test classes that verify the full request pipeline: HTTP request → Controller → Service → Repository → Stored Procedure → Response.
...
2. **Validation**: Invalid DTO → 400 Bad Request
3. **Not found**: Non-existent ID → 404
4. **Auth**: No token → 401 Unauthorized
5. **Tenant isolation**: Other tenant → empty/forbidden
6. **Concurrency**: Simultaneous creates → no duplicates
</step>
...
- Create data in Tenant A, query from Tenant B → empty
- Update data from wrong tenant → not found
- Delete data from wrong tenant → not found
- List endpoints only return own tenant data

**InputValidationTests:**
- SQL injection attempts → rejected (SPOnly helps but verify)
- XSS payloads in string fields → sanitized
- Oversized payloads → 413 or validation error
- Malformed JSON → 400
</step>
