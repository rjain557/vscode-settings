...
src/extensions/{extension-name}/
  shared/                              # Cross-browser core
    background/
      ...
      messaging.ts                     # Message passing (popup ↔ background ↔ content)
    content/
...
- `npm run build:all` → builds all targets
- `npm run dev` → Chrome dev mode with hot reload
- `npm run package` → creates .zip files for store submission
...
4. **Integration tests**: Full message flow (popup → background → API → response)
...
4. Load unpacked in Chrome: chrome://extensions → dist/chrome/
5. Load in Edge: edge://extensions → dist/edge/
6. Safari: Open xcode/{ExtName}.xcodeproj → Run
