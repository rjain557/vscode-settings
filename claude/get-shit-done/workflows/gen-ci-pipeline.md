<purpose>
Generate a complete CI/CD pipeline for your project. Supports GitHub Actions and Azure DevOps Pipelines. Creates build, test, deploy stages with environment promotion (dev → staging → production). Tailored to the Technijian stack: ASP.NET Core 8 API + React/Vite SPA + SQL Server migrations.
...
**Stages:**
1. **Build API**: `dotnet restore` → `dotnet build` → `dotnet publish`
2. **Build SPA**: `npm ci` → `npm run build` → `npm run lint`
3. **Unit Tests**: `dotnet test` (xUnit) + `npm test` (Vitest/Jest)
4. **Integration Tests**: Spin up SQL Server (Docker or LocalDB) → run migrations → run integration tests
...
6. Swap staging → production slot (if blue/green)
